...
caffe.set_device(0)
caffe.set_mode_gpu()
net = caffe.Net('deploy.prototxt', 'VGG_VOC0712_SSD_300x300_iter_3000.caffemodel', caffe.TEST)
...
def process_result(box, conf, cls, cls_dict, conf_th):
    ...
    for bb, cf, cl in zip(box, conf, cls):
        ...
        result.append([cls_name, cf, [x_min, y_min, x_max, y_max, area]])
...
def detect_img(img):
    ...
    caffe.set_mode_gpu()
    box, conf, cls = detect(img, net)
    result = process_result(box, conf, cls, cls_dict, 0.7)
...
if __name__ == "__main__":
    ...
    import os
    import uuid
    for item in os.listdir("ground"):
        name = os.path.join("ground", item)
        img = cv2.imread(name)
